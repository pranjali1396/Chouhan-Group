...
(function() {
  ...
  function getWebsiteConfig() {
    ...
    // Detect Business Center website
    if (hostname.includes('business-center') || hostname.includes('chouhan-business-center')) {
      return {
  ...
  function mapFormToLead(formData, form) {
    ...
    return {
      ...
      remarks: message.trim() || `Inquiry from ${WEBSITE_CONFIG.SOURCE_NAME}. Source: ${source || 'Unknown'}. Property type: ${homeType || 'Not specified'}. Broker: ${broker || 'Not specified'}`,
      metadata: {
  ...
  function init() {
    log('🚀 CRM Integration initialized for:', WEBSITE_CONFIG.SOURCE_NAME);
    ...
    const forms = document.querySelectorAll('form');
      ...
      if (forms.length === 0) {
        log('⚠️ No forms found. Retrying in 1 second...');
      ...
      try {
        ...
        if (hasName || hasPhone || hasEmail) {
          ...
          if (!leadData.customerName && !leadData.mobile) {
            log('⚠️ Warning: No name or phone found');
          }
          
          sendToCRM(leadData).then(result => {
            if (result.success) {
              log('✅ Lead sent to CRM successfully!', result.leadId);
            } else {
              log('❌ Failed to send lead:', result.error);
            }
          }).catch(err => log('❌ Error:', err));
        } else {
          log('⚠️ Form does not appear to be a contact form');
        }
      } catch (error) {
        log('❌ Error handling form:', error);
      }
    }
